...
tmux \
  new-session  "python python_ibft/server.py 0 --offline-delayed --input-value 'decide this'; read" \; \
  split-window "python python_ibft/server.py 1 --online-delayed --input-value 'decide this'; read" \; \
  split-window "python python_ibft/server.py 2 --online-delayed --input-value 'decide this'; read" \; \
